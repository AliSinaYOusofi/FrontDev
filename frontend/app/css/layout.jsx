...
export default function layout({params}) {
    ...
    return (
        <div className={`relative ${theme ? "bg-[#000308] text-white" : "bg-[white] text-black"}`}>
            <Navbar display={eye}/>
            <Sidebar display={eye} topic={params}/>
            <BackToTop />

            <div onClick={() => setEye(prev => !prev)} className="fixed bg-gray-700 text-white p-2 rounded-full top-[91%] right-[5%] hidden md:block cursor-pointer">
                {
                    eye
                    ?
